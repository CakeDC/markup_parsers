...
/**
 * Replace inline images ![alt text](/url/to/img.png) with <img src="/url/to/img.png" alt="alt text"/>
 *
 * @param string $text Text to transform
 * @return string Transformed text.
 */
	protected function _doInlineImage($text) {
		$imagePattern = '/\!\[([^\]]+)\]\s*\(([^ \t]+)([\s\t]*([\"|\'])(.+)\4)?\)/';
		return preg_replace_callback($imagePattern, array($this, '_inlineImageHelper'), $text);
	}

/**
 * Helper function for replacing of inline images
 *
 * @param array $matches regex matches
 * @return string Replaced text
 */
	protected function _inlineImageHelper($matches) {
		$title = null;
		if (isset($matches[5])) {
			$title = ' title="' . $matches[5] . '"';
		}
		return $this->_makePlaceHolder(sprintf('<img src="%s"%s alt="%s"/>', $matches[2], $title, $matches[1]));
	}
